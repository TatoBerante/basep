...
if (!$mysqli) {
  header ('location:../v/default.php?page=aprpay&idr='.$_REQUEST['remito'].'&err=1');
}
else {
  $sql = "UPDATE cirugias cx
          SET cx.estado = ?
          WHERE cx.id_remito = ?";
  ...
  if (!mysqli_stmt_prepare ($stmt, $sql)) print_r (mysqli_stmt_error($stmt));
    ...
    mysqli_stmt_bind_param ($stmt, "ii", $nestado, $rem);
  ...
  */
  mysqli_close($mysqli);
  header ('location:../v/default.php?page=pnlcx&remito='.$rem.'&sent=1');
}
